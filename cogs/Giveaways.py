    ...
    async def start(self, ctx: commands.Context, time: str, winners: int, prize: str):
        ...
        if message:
            try:
                ...
                await timeouts.add(time=time / 1000, id="giveaway", data={ "guild": ctx.guild.id, "channel": ctx.channel.id, "message": message.id, "winners": winners, "prize": prize, "host": ctx.author.id })
